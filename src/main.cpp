...
class HTTPServer {
public:
    HTTPServer(short port) : acceptor_(io_context_, tcp::endpoint(tcp::v4(), port)) {
        startAccept();
    }
...
private:
    ...
    void handleRequest(std::shared_ptr<tcp::socket> socket) {
        try {
            ...
            std::string msg = "Successfully connected on port " + std::to_string(socket->local_endpoint().port());
